...
static float *g_mtp_pred = NULL;    /* MTP-predicted hidden state for next-token expert prefetch */
static int g_mtp_pred_D = 0;
static float *g_layer_states = NULL;  /* per-layer hidden states for MTP pilot prefetch */
static int g_layer_states_n = 0, g_layer_states_D = 0;
#ifdef COLI_CUDA
...
/* Prefetch experts for the next forward pass using per-layer hidden states
 * saved from the CURRENT token's forward pass. Each layer's router runs on the
 * actual state that layer produced last time — zero-change prediction.
 * Optionally adjusts by the MTP delta (final-state prediction). */
static void mtp_pilot_prefetch(Model *m){
    if(!g_pilot) return;
    if(!g_layer_states || g_layer_states_n!=m->c.n_layers || g_layer_states_D!=m->c.hidden){
        free(g_layer_states);
        g_layer_states = calloc((size_t)m->c.n_layers * m->c.hidden, sizeof(float));
        g_layer_states_n = m->c.n_layers;
        g_layer_states_D = m->c.hidden;
    }
    Cfg *c=&m->c; int D=c->hidden, E=c->n_experts, K = g_pilot_k<c->topk ? g_pilot_k : c->topk;
    if(K<1) K=1;
    float *nrm=falloc(D), *ch=falloc(E), *adj=falloc(D);
    int use_mtp = (g_mtp_pred && g_mtp_pred_D==D);
    float *final_save = g_layer_states + (int64_t)(c->n_layers-1)*D;
    for(int l=0; l<c->n_layers; l++){
        Layer *layer=&m->L[l];
        if(!layer->sparse) continue;
        float *src = g_layer_states + (int64_t)l*D;
        if(use_mtp){
            for(int d=0;d<D;d++) adj[d] = src[d] + g_mtp_pred[d] - final_save[d];
            rmsnorm(nrm, adj, layer->post_ln, D, c->eps);
        } else {
            rmsnorm(nrm, src, layer->post_ln, D, c->eps);
        }
        matmul(ch, nrm, layer->router, 1, D, E);
        for(int e=0;e<E;e++) ch[e]=sigmoidf(ch[e])+layer->router_bias[e];
        for(int kk=0;kk<K;kk++){
            int best=0; for(int e=1;e<E;e++) if(ch[e]>ch[best]) best=e;
            ch[best]=-2e30f;
            int found=0;
            pthread_mutex_lock(&g_pilot_mx);
            ESlot *P=m->pin[l];
            for(int z=0;z<m->npin[l] && !found;z++) if(P[z].eid==best) found=1;
            ESlot *Sl=m->ecache[l];
            for(int z=0;z<m->ecn[l] && !found;z++)
                if(Sl[z].eid==best || Sl[z].eid==-(best+2)) found=1;
            pthread_mutex_unlock(&g_pilot_mx);
            if(!found){
                unsigned w=__atomic_load_n(&pilot_w,__ATOMIC_RELAXED);
                if(w-__atomic_load_n(&pilot_r,__ATOMIC_ACQUIRE)<4096){
                    pilot_q[w&4095].l=l; pilot_q[w&4095].e=best;
                    __atomic_store_n(&pilot_w,w+1,__ATOMIC_RELEASE);
                }
            }
        }
    }
    free(nrm); free(ch); free(adj);
}
...
static void layers_forward(Model *m, float *x, int S, int pos_base){
    ...
    int save = (g_layer_states && g_layer_states_n==c->n_layers && g_layer_states_D==D);
    for(int i=0;i<c->n_layers;i++){
        if(S>=8&&(i%4==0||i==c->n_layers-1))
            fprintf(stderr,"[prefill] layer %d/%d · %d token\n",i+1,c->n_layers,S);
        layer_forward(m,&m->L[i],i,x,S,pos_base,nrm,tmp);
        if(save) memcpy(g_layer_states+(int64_t)i*D, x, D*sizeof(float));
    }
    free(nrm); free(tmp);
...
static int mtp_draft(Model *m, int next_tok, int kv, int G, int *draft){
    ...
    for(int g=0;g<G;g++){
        ...
        if(g==0){ if(!g_mtp_pred||g_mtp_pred_D!=D){ free(g_mtp_pred); g_mtp_pred=malloc(D*4); g_mtp_pred_D=D; }
                  memcpy(g_mtp_pred,hx,D*4); }
...
static void profile_print(Model *m, double elapsed){
    ...
    if(g_nn_sub){
        printf("  NN_SUB: attempted %lld / substituted %lld (%.1f%% hit, saved ~%.1f GB I/O)\n",
            (long long)g_nn_attempted,(long long)g_nn_substituted,
            g_nn_attempted>0?100.0*g_nn_substituted/g_nn_attempted:0.,
            g_nn_substituted*18.9e6/1e9);
    }
}
...
#include <fcntl.h>
    ...
    /* Pin: keep all processors in process affinity; pin OpenMP to P-cores,
     * PILOT worker to E-cores via thread affinity. */
    { DWORD_PTR proc_aff, sys_aff;
      ...
      if(cpu_aff_env){ DWORD_PTR m = strtoull(cpu_aff_env, NULL, 16);
          ...
          for(int i = 0, count = 0; i < (int)(sizeof(DWORD_PTR)*8) && count < p_logical; i++)
              if(sys_aff & ((DWORD_PTR)1 << i)){ p_mask |= (DWORD_PTR)1 << i; count++; }
          for(int i = 0; i < (int)(sizeof(DWORD_PTR)*8); i++)
              if((sys_aff & ((DWORD_PTR)1 << i)) && !(p_mask & ((DWORD_PTR)1 << i)))
                  e_mask |= (DWORD_PTR)1 << i;
          g_e_mask = e_mask;
          /* Keep all processors in process affinity; OMP_PLACES=threads maps
           * one logical processor per place. With OMP_PROC_BIND=close and
           * OMP_NUM_THREADS=p_logical, the 16 compute threads bind to the
           * first 16 logical processors (all P-cores 0-15 on i7-12700F).
           * The PILOT worker later binds itself to E-cores via thread affinity. */
          ...
          SetEnvironmentVariable("OMP_PLACES", "threads");
          omp_set_num_threads(p_logical);
          fprintf(stderr, "[AFFINITY] OpenMP %d threads on P-cores, PILOT on %d E-cores, "
              "total %d logical procs (CPU_AFFINITY to override)\n",
              p_logical, e_cores, total);
      }
    }
#endif    
    ...
    nn_init(&m);  /* NN_SUB: compute centroids + build graph (no-op if g_nn_sub=0) */
